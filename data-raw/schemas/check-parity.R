...
# confirms that the clean-room schemas bundled in inst/schemas/ accept and reject
# the same corpus of documents. This is how we keep "behaviorally equivalent to
# upstream" honest without redistributing the upstream files (Google: All Rights
# Reserved; sitemaps.org: CC BY-SA 2.5).
#
...
upstream <- c(
  ...
  "sitemap-video.xsd"    = "https://www.google.com/schemas/sitemap-video/1.1/sitemap-video.xsd",
...
for (f in names(upstream)) {
  ok <- tryCatch(
    utils::download.file(upstream[[f]], file.path(tmp, f), mode = "wb",
                         quiet = TRUE, headers = c("User-Agent" = UA)) == 0L,
...
for (file in names(corpus)) {
  ...
  if (!file.exists(up)) next               # e.g. hreflang: no upstream
  ...
  for (case in corpus[[file]]) {
    ...
    if (!(vu == exp && va == exp && vu == va)) {
      fail <- fail + 1L
      cat(sprintf("[FAIL] %s expected=%s upstream=%s authored=%s\n      %s\n",
                  file, exp, vu, va, substr(xml, 1, 90)))
    }
  }
}
