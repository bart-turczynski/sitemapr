...
Vendored from **canonical upstreams** (not a sibling project), so provenance is
unambiguous. The vendoring is reproducible: `data-raw/schemas/fetch-schemas.R`
is the single source of truth — add a manifest row (or a file under
`data-raw/schemas/authored/`) and re-run to refresh. `data-raw/` is
`.Rbuildignore`'d (SITE-bjkuujcw).

Atomic schemas bundled (one per namespace):
- `sitemap.xsd`, `siteindex.xsd` — core urlset + sitemapindex
  (sitemaps.org, Protocol 0.9)
- `sitemap-image.xsd` (Google 1.1), `sitemap-video.xsd` (Google 1.1),
  `sitemap-news.xsd` (Google 0.9), `sitemap-pagemap.xsd` (Google 1.0)
- `xhtml-hreflang.xsd` — minimal `xhtml:link` element for hreflang alternates.
  **sitemapr-authored**: Google documents hreflang in prose with no dedicated
  XSD, and the full W3C XHTML schema is impractical to import under libxml2.
...
**License/provenance (CRAN blocker):** `inst/schemas/SOURCES.md` (per-file
source URL, namespace, retrieval date, sha256, terms) and `inst/schemas/LICENSE`
ship with the schemas and are regenerated by the fetch script. The Google
schemas are `Copyright Google Inc.`; their redistribution terms must be
confirmed compatible with the package license before CRAN submission — flagged
in `LICENSE` so the call is made explicitly, not discovered late.
...
- Schemas: vendored from canonical upstreams (sitemaps.org, Google) via
  `data-raw/schemas/fetch-schemas.R`, which generates `inst/schemas/SOURCES.md`
  (source URL, sha256, retrieval date, terms) and `inst/schemas/LICENSE` in the
  same commit. sitemaps.org XSDs are CC Attribution-ShareAlike; Google XSDs are
  `Copyright Google Inc.` — confirm redistribution terms before CRAN.
...
- Schema sourcing/licensing: resolved in SITE-bjkuujcw — schemas are vendored
  from canonical upstreams with provenance in `inst/schemas/SOURCES.md`. Open:
  confirm Google XSD redistribution terms are package-license compatible.
